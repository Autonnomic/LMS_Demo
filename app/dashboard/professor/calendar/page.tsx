  ...
  assignments.forEach((a) => {
    ...
    if (d >= start && d <= end) {
      events.push({
        ...
        date: d,
        link: `/dashboard/professor/courses/${a.course_id}/assignments/${a.id}/submissions`,
  ...
  topics.forEach((t) => {
    ...
    if (d >= start && d <= end) {
      events.push({
        ...
        date: d,
        link: `/dashboard/professor/courses/${t.course_id}`,
  ...
  while (cursor <= end) {
    ...
    schedules.forEach((s) => {
      ...
      events.push({
        ...
        link: `/dashboard/professor/courses/${s.course_id}`,
...
export default function ProfessorCalendarPage() {
  ...
  useEffect(() => {
    if (isMobile && viewMode === 'month') setViewMode('week')
  }, [isMobile, viewMode])
  ...
  async function fetchCalendarData() {
    try {
      ...
      // Gather all course IDs where this professor teaches (primary or mapped)
      ...
      const [schedRes, assignRes, topicsRes] = await Promise.all([
        ...
        supabase
          .from('course_topics')
          .select(`
            id, title, scheduled_date, course_id,
            course:courses ( code, name )
          `)
          .in('course_id', courseIds)
      ...
      console.error('Professor calendar fetch error:', e)
    } finally {
      setLoading(false)
    }
  }
  ...
  const selectedKey = useMemo(() => (selectedDate ? dateKey(selectedDate) : null), [selectedDate])
              ...
              <div className="canvas-user-menu-wrapper">
                <div className="canvas-user-menu canvas-user-menu-trigger">
                  ...
                  <div className="canvas-user-menu-name">
                    <div className="skeleton skeleton-text lg" style={{ width: '120px' }} />
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="canvas-content-area">
          ...
          {courseLegend.length > 0 && (
            ...
            >
              ...
              {courseLegend.map((c) => (
                <span key={c.courseId} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.8rem' }}>
                  ...
                  {(() => {
                    ...
                    for (let i = 0; i < startPad; i++) {
                      cells.push(
                        <div
                          key={`pad-${i}`}
                          style={{ minHeight: 100, padding: '0.5rem', background: 'var(--bg)', borderRight: '1px solid var(--border)', borderBottom: '1px solid var(--border)' }}
                        />
                      )
                    }
                      ...
                      cells.push(
                        <div
                          ...
                          style={{
                            ...
                            background: isSelected ? 'rgba(8, 146, 165, 0.12)' : isToday ? 'rgba(8, 146, 165, 0.06)' : 'var(--surface)',
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        padding: '0.75rem',
                        background: isSelected ? 'rgba(8, 146, 165, 0.14)' : isToday ? 'rgba(8, 146, 165, 0.06)' : 'var(--surface-hover)',
                        borderRight: !isMobile && i < 6 ? '1px solid var(--border)' : undefined,
                        boxShadow: isSelected ? '0 0 0 2px var(--teal-bright)' : undefined,
                        cursor: 'pointer',
                      }}
                    >
                      <div style={{ fontWeight: 600, fontSize: '0.875rem', color: 'var(--text-muted)', marginBottom: '0.5rem' }}>
                        {DAY_NAMES[d.getDay()]}
                      </div>
                      <div style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '0.75rem', color: isToday ? 'var(--teal-bright)' : 'var(--text)' }}>
                        {d.getDate()}
                      </div>
          ...
          <div style={{ marginTop: '1.5rem', fontSize: '0.875rem', color: 'var(--text-muted)' }}>
            <strong>Legend:</strong> Class schedules (recurring), assignment deadlines, and course topic dates for your courses.
          </div>
        </div>
      </main>
    </ChatProvider>
